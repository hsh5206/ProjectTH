...
#include "AbilitySystem/AbilityTask/AT_PlayMontageAndWaitForEvent.h"
...
void UAT_PlayMontageAndWaitForEvent::Activate()
{
	...
	UTHAbilitySystemComponent* THAbilitySystemComponent = GetTargetASC();

	if (THAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = THAbilitySystemComponent->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UAT_PlayMontageAndWaitForEvent::OnGameplayEvent));

			if (THAbilitySystemComponent->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
...
void UAT_PlayMontageAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
	...
	if (THAbilitySystemComponent)
	{
		THAbilitySystemComponent->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
