...
#include "AbilitySystem/AbilityTask/AT_ListenCoolDownChange.h"

UAT_ListenCoolDownChange* UAT_ListenCoolDownChange::ListenCooldownChange(UAbilitySystemComponent* AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	UAT_ListenCoolDownChange* ListenCooldownChange = NewObject<UAT_ListenCoolDownChange>();
	ListenCooldownChange->ASC = AbilitySystemComponent;
	ListenCooldownChange->CooldownTags = InCooldownTags;
	ListenCooldownChange->UseServerCooldown = InUseServerCooldown;
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(ListenCooldownChange, &UAT_ListenCoolDownChange::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(ListenCooldownChange, &UAT_ListenCoolDownChange::CooldownTagChanged);
	}

	return ListenCooldownChange;
}

void UAT_ListenCoolDownChange::EndTask()
{
	if (IsValid(ASC))
	{
...
void UAT_ListenCoolDownChange::OnActiveGameplayEffectAddedCallback(UAbilitySystemComponent* Target, const FGameplayEffectSpec& SpecApplied, FActiveGameplayEffectHandle ActiveHandle)
{
...
void UAT_ListenCoolDownChange::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UAT_ListenCoolDownChange::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float& TimeRemaining, float& CooldownDuration)
{
