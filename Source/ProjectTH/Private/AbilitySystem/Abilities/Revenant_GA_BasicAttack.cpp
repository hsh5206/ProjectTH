...
#include "AbilitySystem/Abilities/Revenant_GA_BasicAttack.h"
#include "Engine/SkeletalMeshSocket.h"

#include "Characters/BaseHero.h"
#include "Projectiles/BaseProjectile.h"

void URevenant_GA_BasicAttack::Fire(const FVector& HitTarget)
{
	if (USkeletalMeshComponent* Mesh = Cast<ABaseHero>(GetActorInfo().AvatarActor)->GetMesh())
	{
		const USkeletalMeshSocket* MuzzleFlashSocket = Mesh->GetSocketByName(FName("Muzzle_BaseAttack"));
